    ...
    if st.sidebar.button("Get Recommendations"):
        if user_input_movie:
            try:
                ...
                recommended_movies_collaborative = get_movie_recommendations(user_input_movie, item_similarity_matrix, interaction_data_pivot)
                st.subheader(f"Recommended Movies based on '{user_input_movie}' (Collaborative Filtering):")
                display_movie_info(tmdb_df[tmdb_df['title'].isin(recommended_movies_collaborative[:5])])
            except KeyError:
    ...
    if st.sidebar.button("Advanced Recommendations"):
        if user_preference:
            try:
                ...
                filtered_movies_content_based = tmdb_df[
                ...
                if not filtered_movies_content_based.empty:
                    sorted_movies_content_based = filtered_movies_content_based.sort_values(by='popularity_normalized', ascending=False)
                    recommended_movies_content_based = sorted_movies_content_based['title'].tolist()
                    st.subheader("Recommended Movies based on Advanced Preferences (Content-Based Filtering):")
                    display_movie_info(filtered_movies_content_based[filtered_movies_content_based['title'].isin(recommended_movies_content_based[:5])])
                else:
